Feature: E2E - Task Toaster InvoiceEngine.PreInvoiceValidationMap task (Accent Client)

    // Auto generated by aurea-automation - util on Fri, 24 Sep 2019 13:17:57 GMT
    @HealthCheckSuite
    Scenario: [Accent] User is able to execute "InvoiceEngine.PreInvoiceValidationMap" task in Task Toaster website - [C25734443]
        Given Login to "${taskToasterStaging.url}" with userName : "${stagingCsr.userName}" and password : "${stagingCsr.password}"
        Then Task toaster app is logged in

        When Navigate to URL "${taskToasterStaging.url}" /Task/Settings/425
        Then Tasks Dashboard page is loaded

        When Execute Task for Accent Client
        Then Verify task to finished

        When Click to Accent tab
        Then Verify Log Items loaded

        When Click To Search Button To Refresh Logs
        Then Verify Log Items loaded

        And Verify Log Items for Started
        And Verify Log Items for Complete
